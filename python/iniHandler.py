...
def PrintJSON(type, message, value=""):
	if value == "":
...
def ReadCredentials():
	...
	try:
		...
		PrintJSON("error","Cannot read %s" %credentialsFile)
		sys.exit(1)
	else:
		#Return credentials
		PrintJSON("status", "Read of %s successful." %credentialsFile)
...
def WriteCredentials(id,secret):
	...
	PrintJSON("status", "Writing credentials to %s" %credentialsFile)
	PrintJSON("status", "Writing id: %s and secret: %s" %(id, secret))
	...
	try:
		...
		PrintJSON("error", "%s missing section 'Credentials'" %tokensFile)
		credentialParser.add_section('Credentials')
		with open(iniDirectory + credentialsFile, 'wb') as iniFile:
			credentialParser.write(iniFile)
		WriteCredentials(id,secret)
	else:
		PrintJSON("status", "Credentials write successful")
		
def ReadTokens():
	#Check if tokens.ini exists
	if not FileExists(iniDirectory, tokensFile):
		PrintJSON('error', '%s does not exist' %tokensFile)
		sys.exit(1)
	#Reads tokens from tokens.ini
	PrintJSON("status", "Reading from %s" %tokensFile)
	...
	try:
		...
		PrintJSON("error","Cannot read %s" %tokensFile)
		sys.exit(1)
	else:
		#Return tokens
		PrintJSON("status", "Read of %s successful." %tokensFile)
...
def WriteTokens(AccToken,RefToken):
	...
	PrintJSON("status", "Writing tokens to %s" %tokensFile)
	PrintJSON("status", "Writing access token: %s and refresh token: %s" %(AccToken, RefToken))
	...
	try:
		...
		PrintJSON("error", "%s missing section 'Tokens'" %tokensFile)
		tokenParser.add_section('Tokens')
		with open(iniDirectory + tokensFile, 'wb') as iniFile:
			tokenParser.write(iniFile)
		WriteTokens(AccToken,RefToken)
	else:
		PrintJSON("status", "Write of %s successful." %tokensFile)
